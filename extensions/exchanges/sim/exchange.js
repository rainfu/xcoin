...
module.exports = function sim(conf, so, s) {
  ...
  var real_exchange
  try {
    real_exchange = require(path.resolve(__dirname, `../${so.exchange}/exchange`))(conf, so)
  } catch (e) {
    real_exchange = require(path.resolve(__dirname, '../ccxt/exchange'))(conf, so)
  }
  var now
  ...
  var exchange = {
    ...
    initFees: function () {//User can change the fees from client ,so we sould reset the fees for this
      real_exchange.initFees()
      if (so.future) {
        this.makerFee = real_exchange.makerFee * (so.leverage || 10)
        this.takerFee = real_exchange.takerFee * (so.leverage || 10)
      }
      else {
    ...
    getBalance: function (opts, cb) {
      setTimeout(function () {
        let symbol = so.symbols.find(s => s.currency === opts.currency && s.asset === opts.asset)
    ...
    buy: function (opts, cb) {
      setTimeout(function () {
        if (so.debug) console.log(`\nbuying ${opts.size * opts.price} vs on hold: ${s.balance.currency} - ${s.balance.currency_hold} = ${s.balance.currency - s.balance.currency_hold}`)
        if (opts.size * opts.price > (s.balance.currency - s.balance.currency_hold)) {
    ...
    sell: function (opts, cb) {
      setTimeout(function () {
        if (so.debug) console.log(`selling ${opts.size} vs on hold: ${s.symbols[opts.product_id].asset_amount} - ${s.symbols[opts.product_id].asset_hold} = ${s.symbols[opts.product_id].asset_amount - s.symbols[opts.product_id].asset_hold}`)
        if (opts.size > (s.balance.asset - s.symbols[opts.product_id].asset_hold)) {
          if (so.debug) console.log('nope')
          return cb(null, { status: 'rejected', reject_reason: 'balance' })
        }
    ...
    processTrade: function (trade) {
      ...
      _.each(openOrders, function (order) {
        ...
        if (order.tradetype === 'buy' && (so.mode === 'sim' || trade.price <= order.price)) {
          processBuy(order, trade)
          recalcHold(order.product_id)
        }
        else if (order.tradetype === 'sell' && (so.mode === 'sim' || trade.price >= order.price)) {
  ...
  function processBuy(buy_order, trade) {
    ...
    s.symbols[buy_order.product_id].asset_amount = Number(n(s.symbols[buy_order.product_id].asset_amount).add(size).subtract(fee).format(s.symbols[buy_order.product_id].asset_increment ? s.symbols[buy_order.product_id].asset_increment : so.price_format))
    s.balance.currency = Number(n(s.balance.currency).subtract(total).format(so.price_format))
  ...
  function processSell(sell_order, trade) {
    ...
    if (sell_order.position_side === 'SHORT') {
      let initTotal = n(s.symbols[trade.product_id].last_buy_price).multiply(size)
    ...
    s.symbols[sell_order.product_id].asset_amount = Number(n(s.symbols[sell_order.product_id].asset_amount).subtract(size).format(s.symbols[sell_order.product_id].asset_increment ? s.symbols[sell_order.product_id].asset_increment : so.price_format))
    if (s.symbols[sell_order.product_id].asset_amount < 0) {
      s.symbols[sell_order.product_id].asset_amount = 0
    }
    s.balance.currency = Number(n(s.balance.currency).add(total).subtract(fee).format(so.price_format))
    // Process existing order size changes
