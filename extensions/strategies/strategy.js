var ti = require('../../lib/technicalindicators')
    , { getInputValues, sendSignal, getCrossPoint } = require('../../lib/strategy-helper')
    , z = require('zero-fill')
    , n = require('numbro')
    ...
    var main = {
        calculate(symbol) {
            ...
            so.strategy.strategies.forEach((st) => {
                ...
                if (!symbol.period.strategy) symbol.period.strategy = {
                ...
                if (st.group === 'candlestick') {
                    ...
                    if (st.buyPoint) {
                        for (let i = 0; i < st.inputParams.buyPatterns.length; i++) {
                            ...
                            let buyPatterns = ti[st.inputParams.buyPatterns[i].toLowerCase()](Object.assign(getInputValues(inLookback, st.inputParams.valType), { reversedInput: true }))
                            if (buyPatterns) {
                                symbol.period.strategy.patternname = st.inputParams.buyPatterns[i].toLowerCase()
                    ...
                    if (st.sellPoint) {
                        for (let i = 0; i < st.inputParams.sellPatterns.length; i++) {
                            let inLookback = symbol.lookback.slice(1)
                            let sellPatterns = ti[st.inputParams.sellPatterns[i].toLowerCase()](Object.assign(getInputValues(inLookback, st.inputParams.valType), { reversedInput: true }))
                            if (sellPatterns) {
                                symbol.period.strategy.patternname = st.inputParams.sellPatterns[i].toLowerCase()
                    ...
                    let input = Object.assign({}, st.inputParams, getInputValues(symbol.lookback, st.inputParams.valType), { reversedInput: true })
        ...
        onPeriod(symbol, cb) {
            ...
            if (!symbol.inSignal) {
                let sellSignal = false, buySignal = false
                so.strategy.strategies.forEach((st) => {
                ...
                if (sellSignal) {
                    debug.msg(symbol.product_id.green + " " + so.strategy.name.cyan + ' send signal '.green + 'sell '.cyan)
                    sendSignal(so, symbol, 'sell')
                }
                if (buySignal) {
                    debug.msg(symbol.product_id.green + " " + so.strategy.name.cyan + ' send signal '.green + 'buy '.cyan)
                ...
                if (symbol.period.strategy) {
                    Object.keys(symbol.period.strategy).forEach(key => {
                        ...
                        if (currentOutput && currentOutput.report) {
                            ...
                            cols.push(z(12, n(symbol.period.strategy[key]).format(symbol.increment, Math.floor), ' ').cyan)
                        }
                    })
                }
                return cols
            ...
            if (op.indexOf('cross') >= 0) {
                ...
                if (crossMap.has(currentStrage)) {
                    ...
                    if (op === 'crossUp')
                        ...
                        cross = new ti.CrossDown({ lineA: [], lineB: [] });
