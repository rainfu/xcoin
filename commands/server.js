...
module.exports = function (program, conf) {
  program
    ...
    .allowUnknownOption()
    .description('run another websocket process to get tickers data and excure help method out of the main thred')
    ...
    .action(function (exchangename, cmd) {
      ...
      try {
        exchange = require(path.resolve(__dirname, `../extensions/exchanges/${so.exchange}/exchange`))(conf, so)
      } catch (e) {
      ...
      exchange.refreshProducts((products) => {
        ...
        console.log('\nStart watch filted tickers '.green + products.length.toString().cyan)
        let websocket = require(path.resolve(__dirname, `../lib/ws_server`))(conf, exchange, so)
        websocket.run()
