const collectionService = require('../lib/mongo-service')
    ...
    , { getBacktestData } = require('../extensions/output/api/status')
...
module.exports = function core(s, conf, engine) {
    ...
    function buy(cb, symbol, position_side = 'LONG', size = null, is_taker = false) {
        ...
        engine.executeSignal('buy', function () {
            debug.msg('\nbuySymbol ok')
            if (cb) cb()
        }, size, false, is_taker, false, symbol)
    }
    function buyAll(cb, symbols, size = s.options.max_buy_size) {//TODO:should parse to json
        ...
        let isShort = s.symbols[symbol.product_id].positionSide && s.symbols[symbol.product_id].positionSide === 'SHORT'

        buy(() => {
            setTimeout(function () {
                buyAll(cb, symbols)
            }, 500)
        }, symbol, isShort ? 'SHORT' : 'LONG', size)
    }
    function sellAll(cb, symbols) {
        ...
        let symbol = symbols.pop()
        let isShort = s.symbols[symbol.product_id] && s.symbols[symbol.product_id].positionSide === 'SHORT'
        // console.log('s.symbols[symbol.product_id]', isShort)
        sell(function () {
            setTimeout(function () {
                sellAll(cb, symbols)
            }, 500)
        }, symbol, isShort ? 'SHORT' : 'LONG')
    }
    function sell(cb, symbol, position_side = 'LONG', size = null, is_taker = false, is_part = false) {
        if (position_side === 'SHORT') {
            s.symbols[symbol.product_id].inSignal = 'short'
        ...
        if (is_part) {
            s.symbols[symbol.product_id].last_sell_type = 'manual_sell_' + (position_side === 'short' ? 'short' : 'long')
        }
        console.log('start sellSymbol'.bgGreen, symbol.product_id, position_side, size, is_part)

        engine.executeSignal('sell', function () {
    ...
    function getInitKLine(cb, symbols, symbol, inOpts = { limit: 1000 }) {
        ...
        var query_start = tb().resize(s.symbols[symbol.product_id]._period).subtract(inOpts.limit).toMilliseconds()
        var opts = {
        ...
        console.log(symbol.normalized.green, 'fetching pre-roll klines'.cyan, s.symbols[symbol.product_id]._period, ' start'.cyan, moment(query_start).format('YYYYMMDDHHMM').green, ' limit'.cyan, inOpts.limit)
        s.exchange.getKLines(opts, function (err, klines) {
            if (err) {
                if (err.code === 'ETIMEDOUT' || err.code === 'ENOTFOUND' || err.code === 'ECONNRESET') {
                    if (prev_timeout) {
                        console.error('\n' + moment().format('YYYY-MM-DD HH:mm:ss') + ' - getKLines request timed out. retrying...')
                    }
                    prev_timeout = true
                }
                else if (err.code === 'HTTP_STATUS') {
                    if (prev_timeout) {
                        console.error('\n' + moment().format('YYYY-MM-DD HH:mm:ss') + ' - getKLines request failed: ' + err.message + '. retrying...')
                    }
                    prev_timeout = true
                }
                else {
                    console.error('\n' + moment().format('YYYY-MM-DD HH:mm:ss') + ' - getKLines request failed. retrying...')
                    console.error(err)
                }
                return
            }
            engine.updateKLines(symbol, klines, true, function (err) {
                console.log(symbol.symbol ? symbol.symbol : symbol.product_id.green, 'fetching pre-roll klines ok'.cyan, s.symbols[symbol.product_id]._period.green, ('...' + (s.exchange.getProducts().length - symbols.length) + '/' + s.exchange.getProducts().length).cyan, (klines.length + '').green)
                ...
                engine.syncBalance(function (err) {
                    ...
                    setTimeout(() => {
                        ...
                        console.log('get kline last period'.cyan, symbol.product_id, s.symbols[symbol.product_id].lookback[0].period_id.cyan, moment(s.symbols[symbol.product_id].lookback[0].time).format('MMDD HH:mm:ss').green, moment(s.symbols[symbol.product_id].lookback[0].close_time).format('MMDD HH:mm:ss').green)
    ...
    function getExchangeHotSymbols(options = {
        number: 10,
        limit: 10,
        period: '1d'
    }, cb, client = null) {
        ...
        let getAliveSymbols = (products) => {
            ...
            s.exchange.getKLines(opts, function (err, klines) {
                ...
                hlArray.push(a)
                console.log('getExchangeHotSymbols finished ' + (totalP - products.length) + "/" + totalP)
                console.log('%s %s ,trend:%s,sum:%s,up:%s,cup:%s', opts.product_id, options.limit, a.sum, a.change, a.upCount, a.continueUpCount)
                if (client) {
                    let data = {
                        action: 'watchProcess',
                        data: {
        ...
        let initProducts = _.defaultsDeep(s.exchange.getProducts()).filter(f => {
    ...
    function getExchangeProducts(exchanges = 'all', type = 'USDT', detail = false) {
        if (exchanges === 'all') {
        ...
        let data = exchanges.filter(e => e !== 'sim').map(function (exchange) {
            let ex = {
                name: exchange,
                type
            }
            ...
            if (detail === 'label') {
                ex.products = products.map(p => p.asset + '-' + p.currency).join(',')
