...
module.exports = {
  ...
  normalizeSelector: function (selector) {
    ...
    return parts[0].toLowerCase() + '.' + ((parts[1] || '').indexOf('0x') === 0 ? (parts[1] || '').toLowerCase() : (parts[1] || '').toUpperCase())
  },
  ...
  crossover: function (s, key1, key2) {
    return s.symbols[s.selector.product_id].period[key1] > s.symbols[s.selector.product_id].period[key2] && s.symbols[s.selector.product_id].lookback[0][key1] <= s.symbols[s.selector.product_id].lookback[0][key2]
  },
  ...
  lowest: function (s, key, length) {
    if (s.symbols[s.selector.product_id].lookback.length < length) {
      ...
      s.symbols[s.selector.product_id].period[key] = s.symbols[s.selector.product_id].period[key] = Math.min(s.symbols[s.selector.product_id].period.low, ...s.symbols[s.selector.product_id].lookback.slice(0, length - 1).map(period => period.low))
    }
  },
  highest: function (s, key, length) {
    if (s.symbols[s.selector.product_id].lookback.length < length) {
      ...
      s.symbols[s.selector.product_id].period[key] = s.symbols[s.selector.product_id].period[key] = Math.max(s.symbols[s.selector.product_id].period.high, ...s.symbols[s.selector.product_id].lookback.slice(0, length - 1).map(period => period.high))
    }
  },
  pivot: function (s, leftBars, rightBars) {
    let totalBars = leftBars + rightBars + 1,
      periods = [s.symbols[s.selector.product_id].period, ...s.symbols[s.selector.product_id].lookback.slice(0, totalBars - 1)].reverse(),
