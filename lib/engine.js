...
module.exports = function (s, conf,core) {
    ...
    if (s.symbols[symbol.product_id].my_trades.length) {
      ...
      if (!s.symbols[symbol.product_id].inSignal && !s.symbols[symbol.product_id].acted_on_stop) {
        ...
        if ((s.symbols[symbol.product_id].action && (s.symbols[symbol.product_id].action === 'bought' || s.symbols[symbol.product_id].action === 'partSell')) && (do_sell_stop && s.symbols[symbol.product_id].sell_stop && (s.symbols[symbol.product_id].last_buy_type && (s.symbols[symbol.product_id].last_buy_type.indexOf('short') >= 0 ? s.symbols[symbol.product_id].period.close > s.symbols[symbol.product_id].sell_stop : s.symbols[symbol.product_id].period.close < s.symbols[symbol.product_id].sell_stop)))) {
          logger.warn(symbol.product_id + ' loss_stop_sell_ ' + s.symbols[symbol.product_id].last_buy_type, s.symbols[symbol.product_id].last_buy_price, s.symbols[symbol.product_id].sell_stop, s.symbols[symbol.product_id].period.close)
  ...
  function executeSignal(signal, _cb, size, is_reorder, is_taker, reverseCalled, symbol) {
    ...
    let cb = function (err, order) {
      ...
      logger.info('last action ',s.symbols[symbol.product_id].action)
      if (_cb)
        _cb(err, order)
      else if (err) {
    ...
    syncBalance(function (err, { quote }) {
      ...
      if (is_reorder && s.symbols[symbol.product_id][signal + '_order']) {
        ...
        debug.msg(symbol.product_id + ' price changed, resizing order, ' + reorder_pct + '% remain')
        logger.warn(symbol.product_id + ' price changed, resizing order, ' + reorder_pct + '% remain')
        size = null
      }
      ...
      if (signal === 'buy') {
        ...
        if (so.use_fee_asset) {
          ...
          if (so.future&&so.order==='taker'&&!so.post_only){
            fee = s.exchange.makerFee
          }
          else {
        ...
        if (noEnoughMoney && s.balance.currency_hold > 0) {
          debug.msg(symbol.product_id + ' buy delayed: ' + formatPercent(n(s.balance.currency_hold || 0).divide(s.balance.currency).value()) + ' of funds (' + formatCurrency(s.balance.currency_hold, symbol.currency) + ') on hold')
          logger.warn(symbol.product_id + ' buy delayed: ' + formatPercent(n(s.balance.currency_hold || 0).divide(s.balance.currency).value()) + ' of funds (' + formatCurrency(s.balance.currency_hold, symbol.currency) + ') on hold')
  ...
  function cancelOrder(order, type, do_reorder, cb, symbol) {
    ...
    s.exchange.cancelOrder({ order_id: order.order_id, product_id: symbol.product_id }, function (body, err) {
      if (err) {
        logger.warn(symbol.product_id + ' cancelOrder error', JSON.stringify(err))
        return
      }
      //   debug.msg('start cancel order...')
      logger.info(symbol.product_id + ' cancelOrder ok', JSON.stringify({ order_id: order.order_id, product_id: symbol.product_id }), ' results ', JSON.stringify(body || ''))
