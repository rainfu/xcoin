...
module.exports = function (s, conf) {
  ...
  function initExchange(cb, position_side = "LONG") {
    ...
    try {
      s.exchange.getBalance({
        ...
        }).filter(xs => xs.positionSide === position_side)
  ...
  function cancelOrder(order, type, do_reorder, cb, symbol) {
    ...
    s.exchange.cancelOrder({ order_id: order.order_id, product_id: symbol.product_id }, function (body) {
        ...
        getQuote(function (err, quote) {
          ...
          if (type === 'buy') {
            ...
            if (so.exact_buy_orders && n(order.price).value() != marked_price) {
              ...
              logger.warn(symol.product_id + " " + marked_price + ' exact vs! our ' + order.price)
              cancelOrder(order, type, true, cb, symbol)
            }
            else if (n(order.price).value() < marked_price) {
              debug.msg(marked_price + ' vs our ' + order.price)
              logger.warn(symol.product_id + " " + marked_price + ' vs our ' + order.price)
              cancelOrder(order, type, true, cb, symbol)
            }
            else {
              order.local_time = now()
