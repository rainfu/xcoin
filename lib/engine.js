...
module.exports = function (s, conf) {
  ...
  function initSymbols(symbols) {
    ...
    products.forEach(function (product) {
      symbols.forEach(symbol => {
        if (product.asset === symbol.asset && product.currency === symbol.currency) {
          ...
          if (!s.symbols[symbol.product_id]) {
            ...
            let _period = index >= 0 && so.symbolperiods && so.symbolperiods[index] ? so.symbolperiods[index] : so.period
            ...
            s.symbols[symbol.product_id].leverage = so.leverage || 10
  ...
  try {
    ...
    let strategies = []
    let files = fs.readdirSync(path.resolve(__dirname, '../extensions/strategies/list'))
    for (let i = 0; i < files.length; i++) {
      if (files[i].lastIndexOf('.json') >= 0) {
        let file = require(path.resolve(__dirname, '../extensions/strategies/list/' + files[i]))
    ...
    for (let i = 0; i < files.length; i++) {
      if (files[i].lastIndexOf('.json') >= 0) {
        let file = require(path.resolve(__dirname, '../data/strategies/' + files[i]))
        strategies.push(file)
      }
    }
  ...
  if (so.strategy) {
    ...
    s.strategy = require(path.resolve(__dirname, `../extensions/strategies/strategy`))(so)
  ...
  function nextBuyForQuote(s, quote, symbol) {
    ...
    if (s.next_buy_price) {
      if (s.symbols[symbol.product_id].increment) {
        return n(s.next_buy_price).format(s.symbols[symbol.product_id].increment, Math.floor)
      } else {
      ...
      if (s.symbols[symbol.product_id].inSignal === 'short') {
        if (s.symbols[symbol.product_id].increment) {
          return n(quote.bid).add(n(quote.bid).multiply(s.options.markdown_buy_pct / 100)).format(s.symbols[symbol.product_id].increment, Math.floor)
        } else {
          return n(quote.bid).add(n(quote.bid).multiply(s.options.markdown_buy_pct / 100)).value()

        }
      }
      else {
        if (s.symbols[symbol.product_id].increment) {
          return n(quote.bid).subtract(n(quote.bid).multiply(s.options.markdown_buy_pct / 100)).format(s.symbols[symbol.product_id].increment, Math.floor)
        } else {
          return n(quote.bid).subtract(n(quote.bid).multiply(s.options.markdown_buy_pct / 100)).value()
        }
      }
    }
  }
  ...
  function nextSellForQuote(s, quote, symbol) {
    ...
    if (s.next_sell_price) {
      if (s.symbols[symbol.product_id].increment) {
        return n(s.next_sell_price).format(s.symbols[symbol.product_id].increment, Math.ceil)
      ...
      if (s.symbols[symbol.product_id].inSignal === 'short') {
        if (s.symbols[symbol.product_id].increment) {
          return n(quote.ask).subtract(n(quote.ask).multiply(s.options.markup_sell_pct / 100)).format(s.symbols[symbol.product_id].increment, Math.ceil)
        }
        else {
          return n(quote.ask).subtract(n(quote.ask).multiply(s.options.markup_sell_pct / 100)).value()
        }
      }
      else {
        if (s.symbols[symbol.product_id].increment) {
          return n(quote.ask).add(n(quote.ask).multiply(s.options.markup_sell_pct / 100)).format(s.symbols[symbol.product_id].increment, Math.ceil)
        }
        else {
          return n(quote.ask).add(n(quote.ask).multiply(s.options.markup_sell_pct / 100)).value()
  ...
  function executeStop(do_sell_stop, symbol) {
    ...
    if (s.symbols[symbol.product_id].my_trades.length) {
      var last_trade = s.symbols[symbol.product_id].my_trades[s.symbols[symbol.product_id].my_trades.length - 1]
      if (last_trade.type === 'buy') {
        s.symbols[symbol.product_id].last_trade_worth = (s.symbols[symbol.product_id].period.close - s.symbols[symbol.product_id].last_buy_price) / s.symbols[symbol.product_id].last_buy_price
        if (s.options.future) {
          if (last_trade.position_side === 'short') {
            s.symbols[symbol.product_id].last_trade_worth = (s.symbols[symbol.product_id].last_buy_price - s.symbols[symbol.product_id].period.close) / s.symbols[symbol.product_id].last_buy_price
          }
          s.symbols[symbol.product_id].last_trade_worth = s.symbols[symbol.product_id].last_trade_worth * s.symbols[symbol.product_id].leverage
        }
      }
      else {
        s.symbols[symbol.product_id].last_trade_worth = (s.symbols[symbol.product_id].period.close - s.symbols[symbol.product_id].last_buy_price) / s.symbols[symbol.product_id].last_buy_price
      ...
      // console.log('executeStop do_sell_stop', !s.symbols[symbol.product_id].inSignal, !s.symbols[symbol.product_id].acted_on_stop, s.symbols[symbol.product_id].last_buy_type, s.symbols[symbol.product_id].last_buy_price, s.symbols[symbol.product_id].sell_stop, s.symbols[symbol.product_id].period.close)
      if (!s.symbols[symbol.product_id].inSignal && !s.symbols[symbol.product_id].acted_on_stop) {
        //  console.log('should stop ', s.symbols[symbol.product_id].action, s.symbols[symbol.product_id].sell_stop, s.symbols[symbol.product_id].period.close)
        if (so.profit_stop_enable) {
          s.symbols[symbol.product_id].profit_stop_high = Math.max(s.symbols[symbol.product_id].profit_stop_high || 0, s.symbols[symbol.product_id].last_trade_worth)
          s.symbols[symbol.product_id].profit_stop = profitStop.getProfitStopPrice(symbol)
        }
        if ((s.symbols[symbol.product_id].action && (s.symbols[symbol.product_id].action === 'bought' || s.symbols[symbol.product_id].action === 'partSell')) && (do_sell_stop && s.symbols[symbol.product_id].sell_stop && (s.symbols[symbol.product_id].last_buy_type && (s.symbols[symbol.product_id].last_buy_type.indexOf('short') >= 0 ? s.symbols[symbol.product_id].period.close > s.symbols[symbol.product_id].sell_stop : s.symbols[symbol.product_id].period.close < s.symbols[symbol.product_id].sell_stop)))) {
          // console.log('executeStop do_sell_stop', s.symbols[symbol.product_id].last_buy_type, s.symbols[symbol.product_id].last_buy_price, s.symbols[symbol.product_id].sell_stop, s.symbols[symbol.product_id].period.close)
          ...
          s.symbols[symbol.product_id].last_sell_type = 'loss_stop_sell_' + s.symbols[symbol.product_id].inSignal
          console.log(('\n' + moment(s.symbols[symbol.product_id].period.latest_trade_time).format('YYYY-MM-DD HH:mm:ss') + ' ' + (s.symbols[symbol.product_id].last_sell_type).bgBlack + ', price: ' + s.symbols[symbol.product_id].period.close).green)
        }
        if (!stop_signal && so.profit_win_enable) {
          stop_signal = profitStop.checkProfitWin(symbol)
          if (stop_signal) {
            console.log(('\n' + moment(s.symbols[symbol.product_id].period.latest_trade_time).format('YYYY-MM-DD HH:mm:ss') + ' ' + (s.symbols[symbol.product_id].last_sell_type).bgRed + ', start: ' + formatPercent(s.symbols[symbol.product_id].last_trade_worth) + ' > profit win set, price: ' + s.symbols[symbol.product_id].period.close).green)
          }
        }
        if (!stop_signal && !s.symbols[symbol.product_id].has_profit_stop && so.profit_stop_enable && s.symbols[symbol.product_id].action && (s.symbols[symbol.product_id].action === 'bought' || s.symbols[symbol.product_id].action === 'partSell')) {
          stop_signal = profitStop.checkProfitStop(symbol)
          if (stop_signal) {
            s.symbols[symbol.product_id].has_profit_stop = true
            s.symbols[symbol.product_id].inSignal = (s.symbols[symbol.product_id].last_buy_type && s.symbols[symbol.product_id].last_buy_type.indexOf('short') >= 0 ? 'short' : 'long')
            s.symbols[symbol.product_id].last_sell_type = 'profit_stop_sell_' + s.symbols[symbol.product_id].inSignal
            console.log(('\n' + moment(s.symbols[symbol.product_id].period.latest_trade_time).format('YYYY-MM-DD HH:mm:ss') + ' ' + (s.symbols[symbol.product_id].last_sell_type).bgRed + ', start: ' + formatPercent(s.symbols[symbol.product_id].last_trade_worth) + ' < ' + formatPercent(s.symbols[symbol.product_id].profit_stop) + ' / ' + formatPercent(s.symbols[symbol.product_id].profit_stop_high) + ', price: ' + s.symbols[symbol.product_id].period.close).green)
          }
        }
        /* }
        else {
  ...
  function syncBalance(cb, symbol) {
    if (!symbol) symbol = so.symbols[0]
    let position_side = s.symbols[symbol.product_id].last_buy_type && s.symbols[symbol.product_id].last_buy_type.indexOf('short') >= 0 ? 'SHORT' : "LONG"
    try {
      s.exchange.getBalance({
        ...
        getQuote(function (err, quote) {
          ...
          if (!s.options.future) {
            ...
            s.symbols[symbol.product_id].asset_capital = n(s.symbols[symbol.product_id].asset_amount).multiply(quote.ask).value()
            ...
            s.symbols[symbol.product_id].asset_hold = balance.asset_hold
            s.symbols[symbol.product_id].asset_amount = balance.asset
            s.symbols[symbol.product_id].asset_capital = n(balance.unrealizedProfit).value()
            if (!s.symbols[symbol.product_id].start_price) {
              s.symbols[symbol.product_id].start_price = n(quote.ask).value()
            }
            // console.log('syncBalance.', symbol.product_id, parseInt(s.options.leverage), parseInt(s.symbols[symbol.product_id].leverage))
            if (s.options.mode === 'live' && s.options.leverage && parseInt(s.symbols[symbol.product_id].leverage) !== parseInt(s.options.leverage)) {
  ...
  function initExchange(cb, position_side = "LONG") {
    ...
    try {
      s.exchange.getBalance({
        ...
        if (s.options.future) {
          exitSymbols = exitSymbols.filter(xs => xs.positionSide === position_side)
        }
          ...
          s.exchange.getTickers({ exitSymbols }, function (err, realTickers) {
            ...
            if (realTickers) {
              Object.keys(realTickers).forEach(t => {
                let symbol = exitSymbols.find((s) => s.normalized === realTickers[t].normalized)
                if (symbol) {
  ...
  function placeOrder(type, opts, cb, symbol) {
    ...
    debug.msg('placing ' + type + ' order...' + JSON.stringify(order))
    logger.info('placing ' + type + ' order...' + JSON.stringify(order))
    ...
    s.exchange[type](order_copy, function (err, api_order) {
      ...
      debug.msg(type + ' order placed at ' + formatCurrency(order.price, symbol.currency))
      logger.info(type + ' order placed at ' + formatCurrency(order.price, symbol.currency))
      ...
      setTimeout(function () { checkOrder(order, type, cb, symbol) }, so.order_poll_time)
    })
  }

  function getQuote(cb, symbol) {
    if (!symbol) symbol = so.symbols[0]
    s.exchange.getQuote({ product_id: symbol.product_id }, function (err, quote) {
      if (err) return cb(err)
  ...
  function isOrderLiquidityTooSmall(product, liquid) {
    if (s.exchange.defi && liquid && parseInt(liquid) <= parseInt(s.options.defi.dailyVolumeUSD)) {
      console.log('isOrderLiquidityTooSmall', liquid, parseInt(s.options.defi.dailyVolumeUSD))
      return true
    }
    return false
  }
  ...
  function executeSignal(signal, _cb, size, is_reorder, is_taker, reverseCalled, symbol) {
    if (!symbol) symbol = so.symbols[0]
    let price, expected_fee, buy_pct, sell_pct, trades
    delete s.symbols[symbol.product_id][(signal === 'buy' ? 'sell' : 'buy') + '_order']
    s.symbols[symbol.product_id].last_signal = signal
    if (!is_reorder && s.symbols[symbol.product_id][signal + '_order']) {
      if (is_taker) s.symbols[symbol.product_id][signal + '_order'].order_type = 'taker'
    ...
    syncBalance(function (err, { quote }) {
      ...
      if (is_reorder && s.symbols[symbol.product_id][signal + '_order']) {
        if (signal === 'buy') {
          if (s.options.future) {
            ...
            reorder_pct = n(size).multiply(s.symbols[symbol.product_id].buy_order.price).add(s.symbols[symbol.product_id].buy_order.fee).divide(s.balance.currency).multiply(100)
          }
        } else {
          if (s.options.future) {
            reorder_pct = so.buy_pct
            ...
            reorder_pct = n(size).divide(s.symbols[symbol.product_id].asset_amount).multiply(100)
          }
        }
        debug.msg('price changed, resizing order, ' + reorder_pct + '% remain')
        logger.warn('price changed, resizing order, ' + reorder_pct + '% remain')
        size = null
      }
      ...
      if (signal === 'buy') {
        ...
        if (is_reorder) {
          ...
          buy_pct = so.buy_pct
          if (s.symbols[symbol.product_id].inSignal === 'short' && so.short_buy_pct) {
            buy_pct = so.short_buy_pct
          }
          if (s.symbols[symbol.product_id].last_buy_type) {
            if (s.symbols[symbol.product_id].last_buy_type.indexOf('first_') >= 0) {
              buy_pct = s.options.rain_first_buy_percent
            } else if (s.symbols[symbol.product_id].last_buy_type.indexOf('second_') >= 0) {
              buy_pct = s.options.rain_second_buy_percent
            } else if (s.symbols[symbol.product_id].last_buy_type.indexOf('third_') >= 0) {
              buy_pct = s.options.rain_third_buy_percent
            } else if (s.symbols[symbol.product_id].last_buy_type.indexOf('max_') >= 0) {
              buy_pct = s.options.rain_max_buy_percent
            }
          }
          if (size) {
            buy_pct = parseFloat(n(100 * size).divide(s.balance.currency).format('0.00', Math.floor))
        ...
        if (s.options.future) {
          ...
          if (so.mode === 'live')
            trade_balance = n(s.balance.currency).divide(100).multiply(buy_pct).multiply(n(s.symbols[symbol.product_id].leverage))
          else
          ...
          if (so.mode === 'live')
            tradeable_balance = n(s.balance.currency).divide(100 + fee).multiply(buy_pct).multiply(n(s.symbols[symbol.product_id].leverage))
          else
            tradeable_balance = n(s.balance.currency).divide(100).multiply(buy_pct)
        ...
        expected_fee = n(trade_balance).subtract(tradeable_balance).format(so.price_format, Math.ceil) // round up as the exchange will too
        ...
        if (buy_pct + fee < 100) {
          ...
          size = n(tradeable_balance).divide(price).format(s.symbols[symbol.product_id].asset_increment ? s.symbols[symbol.product_id].asset_increment : so.price_format)
        } else {
          size = n(trade_balance).subtract(expected_fee).divide(price).format(s.symbols[symbol.product_id].asset_increment ? s.symbols[symbol.product_id].asset_increment : so.price_format)
        }
        if (s.symbols[symbol.product_id].max_size && Number(size) > Number(s.symbols[symbol.product_id].max_size)) {
        ...
        if (isOrderLiquidityTooSmall(s.symbols[symbol.product_id], quote.dayVolume)) {
          //console.log('\nbuy cancel order is too small!')
          return cb('order liquidity is too small', null)
        }
        debug.msg('preparing buy order over ' + formatAsset(size, symbol.asset) + ' of ' + formatCurrency(tradeable_balance, symbol.currency) + ' (' + buy_pct + '%) tradeable balance with a expected fee of ' + formatCurrency(expected_fee, symbol.currency) + ' (' + fee + '%)')
        logger.info('preparing buy order over ' + formatAsset(size, symbol.asset) + ' of ' + formatCurrency(tradeable_balance, symbol.currency) + ' (' + buy_pct + '%) tradeable balance with a expected fee of ' + formatCurrency(expected_fee, symbol.currency) + ' (' + fee + '%)')
        ...
        if (so.max_buy_loss_pct && buy_loss > so.max_buy_loss_pct) {
          let err = new Error('\nloss protection')
          err.desc = 'refusing to buy at ' + formatCurrency(price, symbol.currency) + ', buy loss of ' + formatPercent(buy_loss / 100)
          return cb(err)
        }
        // console.log('so.max_slippage_pct', s.symbols[symbol.product_id].buy_order, so.max_slippage_pct)
        if (s.symbols[symbol.product_id].buy_order && so.max_slippage_pct != null) {
          let slippage = n(price).subtract(s.symbols[symbol.product_id].buy_order.orig_price).divide(s.symbols[symbol.product_id].buy_order.orig_price).multiply(100).value()
          if (s.symbols[symbol.product_id].inSignal === 'short') {
            slippage = n(s.symbols[symbol.product_id].buy_order.orig_price).subtract(price).divide(price).multiply(100).value()
          }
          ...
          if (so.max_slippage_pct != null && slippage > so.max_slippage_pct) {
            let err = new Error('\nslippage protection')
            err.desc = 'refusing to buy at ' + formatCurrency(price, symbol.currency) + ', slippage of ' + formatPercent(slippage / 100)
            return cb(err)
          }
        }
        ...
        let noEnoughMoney = n(s.balance.currency).subtract(s.balance.currency_hold || 0).value() < n(price).multiply(size).value()
        if (s.options.future) {
          noEnoughMoney = n(s.balance.currency).subtract(s.balance.currency_hold || 0).value() < n(price).multiply(size).value() / s.symbols[symbol.product_id].leverage
        }
        if (noEnoughMoney && s.balance.currency_hold > 0) {
          debug.msg('buy delayed: ' + formatPercent(n(s.balance.currency_hold || 0).divide(s.balance.currency).value()) + ' of funds (' + formatCurrency(s.balance.currency_hold, symbol.currency) + ') on hold')
          logger.warn('buy delayed: ' + formatPercent(n(s.balance.currency_hold || 0).divide(s.balance.currency).value()) + ' of funds (' + formatCurrency(s.balance.currency_hold, symbol.currency) + ') on hold')
        ...
        if (is_reorder) {
          ...
          if (s.symbols[symbol.product_id].last_sell_type) {
            if (s.symbols[symbol.product_id].last_sell_type.indexOf('first_') >= 0) {
              sell_pct = s.options.profit_win_first_percent
            } else if (s.symbols[symbol.product_id].last_sell_type.indexOf('second_') >= 0) {
              sell_pct = s.options.profit_win_second_percent
            } else if (s.symbols[symbol.product_id].last_sell_type.indexOf('third_') >= 0) {
              sell_pct = s.options.profit_win_third_percent
            } else if (s.symbols[symbol.product_id].last_sell_type.indexOf('max_') >= 0) {
              sell_pct = s.options.profit_win_max_percent
            } else if (s.symbols[symbol.product_id].last_sell_type.indexOf('profit_stop_') >= 0) {
        ...
        if (size) {
          sell_pct = parseFloat(n(100 * size).divide(s.symbols[symbol.product_id].asset_amount).format('0.00', Math.floor))
          if (sell_pct > so.sell_pct) {
            sell_pct = so.sell_pct
          }
        }

        size = n(s.symbols[symbol.product_id].asset_amount).multiply(sell_pct / 100).format(s.symbols[symbol.product_id].asset_increment ? s.symbols[symbol.product_id].asset_increment : so.price_format)
        ...
        if (s.options.future) {//TODO:win first sell all
          if (s.symbols[symbol.product_id].min_size && n(s.symbols[symbol.product_id].asset_amount).subtract(size).value() > 0 && n(s.symbols[symbol.product_id].asset_amount).subtract(size).value() <= s.symbols[symbol.product_id].min_size) {
            let oldSize = size
            size = n(s.symbols[symbol.product_id].asset_amount).format(s.symbols[symbol.product_id].asset_increment ? s.symbols[symbol.product_id].asset_increment : so.price_format)
          ...
          if (s.symbols[symbol.product_id].min_size && n(s.symbols[symbol.product_id].asset_amount).subtract(size).value() > 0 && n(s.symbols[symbol.product_id].asset_amount).subtract(size).value() <= s.symbols[symbol.product_id].min_size) {
            let oldSize = size
            size = n(s.symbols[symbol.product_id].asset_amount).format(s.symbols[symbol.product_id].asset_increment ? s.symbols[symbol.product_id].asset_increment : so.price_format)
            console.log('Sell spot size'.green, ' from oldSize ', oldSize, ' resize to...', size)
          }
          if (so.min_buy_size && n(s.symbols[symbol.product_id].asset_amount).subtract(size).multiply(price).value() <= so.min_buy_size) {
            let oldSize = size
            //  console.log('resize to...')
            size = n(s.symbols[symbol.product_id].asset_amount).format(s.symbols[symbol.product_id].asset_increment ? s.symbols[symbol.product_id].asset_increment : so.price_format)
            console.log('Sell min_buy_size'.green, ' from oldSize ', oldSize, ' resize to...', size)
          }
        }
        if (s.symbols[symbol.product_id].max_size && Number(size) > Number(s.symbols[symbol.product_id].max_size)) {
          size = n(s.symbols[symbol.product_id].max_size).format(s.symbols[symbol.product_id].asset_increment ? s.symbols[symbol.product_id].asset_increment : so.price_format)
        }
        ...
        if (latest_high_buy && so.sell_cancel_pct != null && Math.abs(sell_loss) < so.sell_cancel_pct) {
          console.log(('\nsell_cancel_pct: refusing to sell at ' + formatCurrency(latest_high_buy.price, symbol.currency) + '-' + formatCurrency(price, symbol.currency) + ', sell loss of ' + formatPercent(sell_loss / 100) + ' - ' + formatPercent(so.sell_cancel_pct / 100) + '\n').red)
          return cb(null, null)
        }
        //    console.log('executeSignal 3')
        if (so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct) {
          console.log('\nso.max_sell_loss_pct', so.max_sell_loss_pct, so.max_sell_loss_pct === null)
          let err = new Error('\nloss protection')
          err.desc = 'refusing to sell at ' + formatCurrency(price, symbol.currency) + ', sell loss of ' + formatPercent(sell_loss / 100)
          return cb(err)
        }
        // console.log('so.max_slippage_pct', s.symbols[symbol.product_id].sell_order, so.max_slippage_pct, price)
        if (s.symbols[symbol.product_id].sell_order && so.max_slippage_pct != null) {
          let slippage = n(s.symbols[symbol.product_id].sell_order.orig_price).subtract(price).divide(price).multiply(100).value()
          if (s.symbols[symbol.product_id].inSignal === 'short') {
            slippage = n(price).subtract(s.symbols[symbol.product_id].sell_order.orig_price).divide(s.symbols[symbol.product_id].sell_order.orig_price).multiply(100).value()
          }
          // console.log('slippage sell', s.symbols[symbol.product_id].inSignal, slippage)
          if (slippage > so.max_slippage_pct) {
            let err = new Error('\nslippage protection')
            err.desc = 'refusing to sell at ' + formatCurrency(price, symbol.currency) + ', slippage of ' + formatPercent(slippage / 100)
            return cb(err)
          }
        }
        if (n(s.symbols[symbol.product_id].asset_amount).subtract(s.symbols[symbol.product_id].asset_amount_hold || 0).value() < n(size).value()) {
          debug.msg(symbol.product_id + ' sell delayed: asset_amount:' + (s.symbols[symbol.product_id].asset_amount || 0) + ",asset_amount_hold:" + (s.symbols[symbol.product_id].asset_amount_hold || 0) + ',sell size:' + (n(size).value()))
          logger.warn(symbol.product_id + ' sell delayed: asset_amount:' + (s.symbols[symbol.product_id].asset_amount || 0) + ",asset_amount_hold:" + (s.symbols[symbol.product_id].asset_amount_hold || 0) + ',sell size:' + (n(size).value()))
          size = n(s.symbols[symbol.product_id].asset_amount).format(s.symbols[symbol.product_id].asset_increment ? s.symbols[symbol.product_id].asset_increment : so.price_format)
          /* return setTimeout(function () {
            if (s.symbols[symbol.product_id].last_signal === signal) {
              executeSignal(signal, cb, size, true, false, false, symbol)
    ...
    function doOrder() {
      placeOrder(signal, {
        ...
        is_taker: is_taker,
        position_side: s.symbols[symbol.product_id].inSignal === 'short' ? 'SHORT' : "LONG",
        cancel_after: so.cancel_after || 'day'
      }, function (err, order) {
        if (err) {
        ...
        if (!order) {
          if (order === false) {
            ...
            logger.warn('not enough balance, or signal switched, cancel ' + signal)
            return cb(null, null)
          }
          if (s.symbols[symbol.product_id].last_signal !== signal) {
          ...
          let remaining_size = s.symbols[symbol.product_id][signal + '_order'] ? s.symbols[symbol.product_id][signal + '_order'].remaining_size : size
          if (remaining_size !== size) {
            debug.msg('remaining size: ' + remaining_size)
            logger.warn('remaining size: ' + remaining_size)
          }
          return executeSignal(signal, _cb, remaining_size, true, false, false, symbol)
        }
        cb(null, order)
      }, symbol)
    }
  ...
  function executeOrder(order, trade_type, symbol) {
    ...
    s.symbols[symbol.product_id].action = trade_type === 'sell' ? 'sold' : 'bought'
    if (trade_type === 'sell' && s.symbols[symbol.product_id].last_sell_type && s.symbols[symbol.product_id].last_sell_type.indexOf('profit_win') >= 0) {
      s.symbols[symbol.product_id].action = 'partSell'
    } else if (trade_type === 'sell' && s.symbols[symbol.product_id].last_sell_type && s.symbols[symbol.product_id].last_sell_type.indexOf('profit_stop') >= 0 && s.options.profit_stop_percent < 100) {
      s.symbols[symbol.product_id].action = 'partSell'
    }
    if (trade_type === 'sell' && s.symbols[symbol.product_id].min_size && s.symbols[symbol.product_id].asset_amount <= s.symbols[symbol.product_id].min_size) {
      s.symbols[symbol.product_id].action = 'sold'
      console.log('change to sold by min_size'.green, s.symbols[symbol.product_id].min_size, 'remain asset'.green, s.symbols[symbol.product_id].asset_amount)
    }
    ...
    let usdtProfit = 0
    if (trade_type === 'sell') {
      if (s.options.future && s.options.paper) {
        usdtProfit = n(price).subtract(s.symbols[symbol.product_id]['last_buy_price']).multiply(order.size).multiply(s.symbols[symbol.product_id].leverage).subtract(fee).value()
        profit = n(usdtProfit).divide(n(s.symbols[symbol.product_id]['last_buy_price']).multiply(order.size)).value()
        if (s.symbols[symbol.product_id].last_buy_type && s.symbols[symbol.product_id].last_buy_type.indexOf('short') >= 0) {
          usdtProfit = n(s.symbols[symbol.product_id]['last_buy_price']).subtract(price).multiply(order.size).multiply(s.symbols[symbol.product_id].leverage).subtract(fee).value()
          profit = n(usdtProfit).divide(n(s.symbols[symbol.product_id]['last_buy_price']).multiply(order.size)).value()
        }
      }
      else {
        usdtProfit = n(price).subtract(s.symbols[symbol.product_id]['last_buy_price']).multiply(order.size).subtract(fee).value()
        profit = n(usdtProfit).divide(n(s.symbols[symbol.product_id]['last_buy_price']).multiply(order.size)).value()
        if (s.symbols[symbol.product_id].last_buy_type && s.symbols[symbol.product_id].last_buy_type.indexOf('short') >= 0) {
          usdtProfit = n(s.symbols[symbol.product_id]['last_buy_price']).subtract(price).multiply(order.size).subtract(fee).value()
          profit = n(usdtProfit).divide(n(s.symbols[symbol.product_id]['last_buy_price']).multiply(order.size)).value()
        }
      }
    }
    ...
    if (trade_type === 'sell' && s.symbols[symbol.product_id].action === 'partSell') {
      s.symbols[symbol.product_id]['lastBuyUsdtprofit'] += usdtProfit
    }
    if (trade_type === 'sell' && s.symbols[symbol.product_id].action === 'sold') {
      s.symbols[symbol.product_id]['lastBuyUsdtprofit'] += usdtProfit
      ...
      s.symbols[symbol.product_id].winLostRate = (s.symbols[symbol.product_id].win + s.symbols[symbol.product_id].lost) > 0 ? (s.symbols[symbol.product_id].win / (s.symbols[symbol.product_id].win + s.symbols[symbol.product_id].lost)) : 0
    }
    ...
    if (trade_type === 'sell' && so.same_period_multi_buy) {
      s.symbols[symbol.product_id].last_sell_period = s.symbols[symbol.product_id].period.period_id
    }
    let my_trade = {
    ...
    if (so.stats) {
      ...
      let total_price = formatCurrency(my_trade.size * my_trade.price, symbol.currency)
    ...
    if (trade_type === 'buy' && so.sell_stop_pct) {
      s.symbols[symbol.product_id].sell_stop = n(price).subtract(n(price).multiply(so.sell_stop_pct / 100)).value()
      if (s.symbols[symbol.product_id].inSignal === 'short') {
        s.symbols[symbol.product_id].sell_stop = n(price).add(n(price).multiply(so.sell_stop_pct / 100)).value()
      }
    } else if (trade_type === 'sell' && so.buy_stop_pct) {
      if (s.symbols[symbol.product_id].last_sell_type && s.symbols[symbol.product_id].last_sell_type.indexOf('profit_stop_sell') >= 0) {
        s.symbols[symbol.product_id].buy_stop = n(price).add(n(price).multiply(so.buy_profit_pct / 100)).value()
        if (s.symbols[symbol.product_id].inSignal === 'short') {
          s.symbols[symbol.product_id].buy_stop = n(price).subtract(n(price).multiply(so.buy_profit_pct / 100)).value()
        }
      }
      else {
        s.symbols[symbol.product_id].buy_stop = n(price).add(n(price).multiply(so.buy_stop_pct / 100)).value()
        if (s.symbols[symbol.product_id].inSignal === 'short') {
          s.symbols[symbol.product_id].buy_stop = n(price).subtract(n(price).multiply(so.buy_stop_pct / 100)).value()
        }
      }
  ...
  function writeReport(is_progress, blink_off, symbol) {
    ...
    // console.log('symbol', symbol)
    process.stdout.write(('   ' + moment(s.symbols[symbol.product_id].period.latest_trade_time).format('MM-DD HH:mm:ss'))[is_progress && !blink_off ? 'bgBlue' : 'cyan'])
    process.stdout.write(z(16, symbol.symbol ? symbol.symbol : symbol.product_id, ' ').green)
    process.stdout.write('        ' + formatCurrency(s.symbols[symbol.product_id].period.close, symbol.currency, true, true, true))
    ...
    if (s.symbols[symbol.product_id].buy_order) {
      ...
      process.stdout.write(z(16, s.symbols[symbol.product_id].action, ' ')[s.symbols[symbol.product_id].action === 'bought' ? 'green' : 'red'])
    }
    else if (s.symbols[symbol.product_id].signal) {
      process.stdout.write(z(16, s.symbols[symbol.product_id].signal || '', ' ')[s.symbols[symbol.product_id].signal ? s.symbols[symbol.product_id].signal === 'buy' ? 'green' : 'red' : 'cyan'])
    }
    else if (s.symbols[symbol.product_id].last_trade_worth && !s.symbols[symbol.product_id].buy_order && !s.symbols[symbol.product_id].sell_order) {
      process.stdout.write(z(16, formatPercent(s.symbols[symbol.product_id].last_trade_worth), ' ')[s.symbols[symbol.product_id].last_trade_worth > 0 ? 'green' : 'red'])
    }
    else {
    ...
    if (start_price) {
      if (!s.options.future) {
        process.stdout.write(z(16, n(s.symbols[symbol.product_id].usdtProfit).format('0.00'), ' ')[(s.symbols[symbol.product_id].usdtProfit || 0) >= 0 ? 'green' : 'red'])
        if (s.symbols[symbol.product_id].last_trade_worth) {
          process.stdout.write(z(16, s.symbols[symbol.product_id].last_trade_worth ? formatPercent(s.symbols[symbol.product_id].last_trade_worth) : ' ', ' ')[s.symbols[symbol.product_id].last_trade_worth >= 0 ? 'green' : 'red'])
          process.stdout.write(z(16, s.symbols[symbol.product_id].profit_stop_high ? formatPercent(s.symbols[symbol.product_id].profit_stop_high) : ' ', ' '))
          process.stdout.write(z(16, s.symbols[symbol.product_id].profit_stop ? formatPercent(s.symbols[symbol.product_id].profit_stop) : ' ', ' '))
        }
      } else {
        process.stdout.write(z(16, n(s.symbols[symbol.product_id].usdtProfit).format('0.00'), ' ')[(s.symbols[symbol.product_id].usdtProfit || 0) >= 0 ? 'green' : 'red'])
        if (s.symbols[symbol.product_id].last_trade_worth) {
  ...
  function refreshBotData() {
    let botData = { tradeListLen: 0, tradeNum: 0, dynamicUsdtProfit: 0, dynamicProfit: 0, usdtProfit: 0, profit: 0, win: 0, lost: 0, winLostRate: 0 }
    ...
    Object.keys(s.symbols).map(key => {
      ...
      if (!so.future && pair.period) {
        botData.totalCapital += n(pair.asset_amount).multiply(pair.period.close).value()
      }
    })
    if (so.future) {
      botData.totalCapital = botData.startCapital + botData.usdtProfit + botData.dynamicUsdtProfit
    }
    ...
    botData.winLostRate = (botData.win + botData.lost) > 0 ? (botData.win / (botData.win + botData.lost)) : 0
  ...
  function writeTotalReport() {
    ...
    process.stdout.write((moment((new Date()).getTime()).format('MM-DD HH:mm:ss'))['bgBlue'])
    ...
    process.stdout.write(z(10, s.status.tradeListLen + '/' + so.symbols.length, ' ').green)
    process.stdout.write(z(10, n(s.status.startCapital).format('0.0'), ' '))
    process.stdout.write(z(10, n(s.status.usdtProfit).format('0.00'), ' ')[s.status.usdtProfit > 0 ? 'green' : 'red'])
    process.stdout.write(z(10, formatPercent(s.status.profit), ' ')[s.status.profit >= 0 ? 'green' : 'red'])
    process.stdout.write(z(10, n(s.status.dynamicUsdtProfit).format('0.00'), ' ')[s.status.dynamicUsdtProfit >= 0 ? 'green' : 'red'])
  ...
  function cancelOrder(order, type, do_reorder, cb, symbol) {
    ...
    s.exchange.cancelOrder({ order_id: order.order_id, product_id: symbol.product_id }, function (body, err) {
      if (err) {
        logger.warn('cancelOrder error', JSON.stringify(err))
        return
      }
      //   debug.msg('start cancel order...')
      logger.info('cancelOrder', JSON.stringify({ order_id: order.order_id, product_id: symbol.product_id }), ' results ', JSON.stringify(body || ''))
      function checkHold(do_reorder, cb) {
        s.exchange.getOrder({ order_id: order.order_id, product_id: symbol.product_id }, function (err, api_order) {
          // console.log(' cancel order ok ...', s.exchange, api_order)
          logger.info('getCancelOrder', JSON.stringify(api_order))
          if (api_order) {
            if (api_order.status === 'done') {
              ...
              logger.info('order done,' + JSON.stringify(api_order))
            ...
            if (api_order.filled_size) {
              order.remaining_size = n(order.size).subtract(api_order.filled_size).format(s.symbols[symbol.product_id].asset_increment ? s.symbols[symbol.product_id].asset_increment : so.price_format)
            }
          }
          syncBalance(function () {
            ...
            if (!s.options.future) {
              ...
              else on_hold = n(s.symbols[symbol.product_id].asset_amount).subtract(s.symbols[symbol.product_id].asset_amount_hold || 0).value() < n(order.remaining_size).value()
            }
            logger.info('on_hold', type, on_hold)
            if (on_hold && s.balance.currency_hold > 0) {
              // wait a bit for settlement
              debug.msg(symbol.product_id + ' funds on hold after cancel, waiting 5s')
              logger.warn(symbol.product_id + ' funds on hold after cancel, waiting 5s')
  ...
  function checkOrder(order, type, cb, symbol) {
    ...
    if (s.options.max_check_order_num && order.checkNum && order.checkNum > s.options.max_check_order_num) {
      debug.msg('order checknum > ' + s.options.max_check_order_num + ', aborting')
      logger.warn('order checknum > ' + s.options.max_check_order_num + ', aborting')
      return cancelOrder(order, type, false, cb, symbol)
    }
    s.exchange.getOrder({ order_id: order.order_id, product_id: symbol.product_id }, function (err, api_order) {
      if (err) return cb(err)
      ...
      if (order.status === 'rejected' && order.reject_reason === 'balance') {
        debug.msg('not enough balance for ' + type + ' ', symbol.product_id, ', aborting')
        logger.warn('not enough balance for ' + type + ' ' + symbol.product_id, ', aborting')
        return cb(null, null)
      }
      if (now() - order.local_time >= so.order_adjust_time) {
        getQuote(function (err, quote) {
          if (err) {
            ...
            logger.warn('could not execute ' + type + ': error fetching quote')
            return cb(err)
          }
          let marked_price
          if (type === 'buy') {
            ...
            if (so.exact_buy_orders && n(order.price).value() != marked_price) {
              ...
              logger.warn(symbol.product_id + " " + type + " " + marked_price + ' exact vs! our ' + order.price)
              cancelOrder(order, type, true, cb, symbol)
            }
            else if (n(order.price).value() < marked_price) {
              debug.msg(marked_price + ' vs our ' + order.price)
              logger.warn(symbol.product_id + " " + type + " " + marked_price + ' vs our ' + order.price)
              cancelOrder(order, type, true, cb, symbol)
            }
            else {
              order.local_time = now()
            ...
            if (so.exact_sell_orders && n(order.price).value() != marked_price) {
              debug.msg(marked_price + ' exact vs! our ' + order.price)
              logger.warn(symbol.product_id + " " + type + " " + marked_price + ' exact vs our ' + order.price)
              cancelOrder(order, type, true, cb, symbol)
            }
            else if (n(order.price).value() > marked_price) {
  ...
  var klineProcessingQueue = async.queue(function ({ symbol, kline, is_preroll }, callback) {
  ...
  function initKLinePeriod(symbol, trade) {
    ...
    if (trade.isTrade) {
      trade.period_id = tb(trade.time).resize(s.symbols[symbol.product_id]._period).toString()
      // console.log('\nget trade'.cyan, trade.period_id.cyan, moment(trade.time).format('MMDD HH:mm:ss').green)
      const inPeriod = s.symbols[symbol.product_id].period && (trade.period_id === s.symbols[symbol.product_id].period.period_id)
      if (inPeriod) {
        ...
        s.symbols[symbol.product_id].period.high = Math.max(trade.close, s.symbols[symbol.product_id].period.high)
        s.symbols[symbol.product_id].period.low = Math.min(trade.close, s.symbols[symbol.product_id].period.low)
        ...
        s.symbols[symbol.product_id].lookback.unshift(s.symbols[symbol.product_id].period)
      }
    ...
    if (s.symbols[symbol.product_id].lookback.length > s.options.keep_lookback_periods) {
  ...
  function onKLine(symbol, kline, is_preroll, cb) {
    ...
    if (s.symbols[symbol.product_id].in_preroll) {//preload
  ...
  function withOnKLinePeriod(symbol, kline, cb) {
    ...
    s.strategy.onPeriod(s.symbols[symbol.product_id], function () {
      ...
      if (s.symbols[symbol.product_id].last_buy_price) {
        s.symbols[symbol.product_id].dynamicProfit = s.symbols[symbol.product_id].last_buy_type.indexOf('short') >= 0 ? (n(s.symbols[symbol.product_id].last_buy_price).subtract(s.symbols[symbol.product_id].period.close)).divide(s.symbols[symbol.product_id].last_buy_price).value() : (n(s.symbols[symbol.product_id].period.close).subtract(s.symbols[symbol.product_id].last_buy_price)).divide(s.symbols[symbol.product_id].last_buy_price).value()
        s.symbols[symbol.product_id].dynamicUsdtProfit = (n(s.symbols[symbol.product_id].dynamicProfit).multiply(s.symbols[symbol.product_id].last_buy_price).multiply(s.symbols[symbol.product_id].asset_amount)).value()
        if (s.options.future) {
          s.symbols[symbol.product_id].dynamicProfit = n(s.symbols[symbol.product_id].dynamicProfit).multiply(s.symbols[symbol.product_id].leverage).value()
          if (s.options.paper) {
            s.symbols[symbol.product_id].dynamicUsdtProfit = n(s.symbols[symbol.product_id].dynamicProfit).multiply(s.symbols[symbol.product_id].last_buy_price).multiply(s.symbols[symbol.product_id].asset_amount).value()
          }
          else {
            s.symbols[symbol.product_id].dynamicUsdtProfit = n(s.symbols[symbol.product_id].dynamicProfit).multiply(s.symbols[symbol.product_id].last_buy_price).multiply(s.symbols[symbol.product_id].asset_amount).divide(s.symbols[symbol.product_id].leverage).value()
          }
        }
      }
      ...
      if (!s.symbols[symbol.product_id].in_preroll) {
        ...
        if (so.trade_type !== 'manual') {
          ...
          if (s.symbols[symbol.product_id].signal) {
            if (so.trade_type === 'auto' || (so.trade_type === 'autoSell' && s.symbols[so.symbols[0].product_id].signal === 'sell') || (so.trade_type === 'autoBuy' && s.symbols[so.symbols[0].product_id].signal === 'buy')) {
              let tempSignal = s.symbols[symbol.product_id].signal
              let tempInSignal = s.symbols[symbol.product_id].inSignal
              executeSignal(s.symbols[symbol.product_id].signal, (err) => {
                ...
                if (err) {
                  ...
                  if (tempSignal === 'sell' && so.market === 'both' && so.buy_position_side_when_sell) {
                    if (s.symbols[symbol.product_id].last_sell_type.indexOf('loss_') >= 0 || s.symbols[symbol.product_id].last_sell_type.indexOf('manual_') >= 0 || s.symbols[symbol.product_id].last_sell_type.indexOf('profit_') >= 0) return
                    console.log('buy_position_side_when_sell'.red, symbol.product_id)
                    s.symbols[symbol.product_id].inBuyPositionSide = true
                    sendAction(s.symbols[symbol.product_id], so.strategy.name, 'buy', tempInSignal === 'long' ? 'short' : 'long')
                    return
                  }
                }
              }, null, false, false, false, symbol)
            }
          }
      ...
      s.symbols[symbol.product_id].last_period_id = s.symbols[symbol.product_id].period.period_id
      if (_.isFunction(cb)) cb()
    })
    // cb()
  }
